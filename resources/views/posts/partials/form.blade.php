{{--  Basic input box to gather the title --}}
...
{{-- displaying especfic error message under title --}}
@error('title')
<div class="alert alert-danger">{{ $message }}</div>
    
@enderror

 {{-- Basic textare box to gather content  --}}
<div class="form-group">
    <label for="content"> Content</label>
    <textarea class="form-control" id="content" name="content" >{{ old('content' , optional($post ?? null)->content)}}</textarea>
</div>

{{-- if error that does not follow the validator then it would display it --}}
@if ($errors->any())
<div class="mb-3 ">
    <ul class="list-group">
